(function() {
  ...
  /** @ngInject */
  function TopUpService($ionicModal, $rootScope, $timeout, $state, OrderService, CouponService, UserService, PayApi, utils, toastr, md5, user, $log, AccApi, TrdApi, $ionicHistory) {
    var self = this,
      order = {},
      modals = [],
    ...
    self.showPayModal = function() {
      ...
      AccApi.getBankCard({
        ...
        if (res.succeed) {
          ...
          mScope.data.needPay = order.needPay;
        }
      });
    ...
    function infoPrompt(index) {
      ...
      var bankCardNo = order.eBank.bankCardNo;
      // if (index == 1) {
      //   var Url2 = document.getElementById("accountNo");
      //   Url2.select();
      ...
      var prompt = [
        ...
        { 'title': '支付宝转账', 'content': '<div class="text-left"><p>自2016年10月12日起，支付宝对个人用户超出免费额度的提现收取0.1%的服务费，个人用户每人累计享有2万元基础免费提现额度。</p><p>第一步：点击支付宝所在模块立即前往文字（徽商账号已被复制），进入支付宝页面;</p><p>第二步：点击转账---转账到银行卡--输入姓名、徽商银行电子交易账户账号、选择“徽商银行”、输入转账金额，完成转账操作（付款方式可选择账户余额或银行储蓄卡，余额宝不支持转账到本人银行卡服务）。</p></div>' },
        { 'title': '网银转账', 'content': '<p>网银转账只能在电脑端操作</p>' },
        { 'title': '柜面转账', 'content': '<div class="text-left"><p>银行柜台转账需填写信息</p><p>收款人:' + realName + '</p><p>收款账号（徽商银行电子交易账号）:</p><p>' + bankCardNo + '</p><p>收款开户行：徽商银行股份有限公司合肥花园街支行</p></div>' }
      ]
      closeModal();
