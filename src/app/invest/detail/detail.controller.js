(function() {
  ...
  function InvestDetailController($scope, $state, $stateParams, BasicApi, OrderService, user, AppService, toastr, $rootScope, EbankService, AgreementService, $log) {
    var vm = this,
      slider,
      investPageIndex = 1,
      borPageIndex = 1,
      ...
      BasicApi.getFinanceProductDetails({
        ...
        .success(function(res) {
          if (+res.flag === 1) {
            ...
            info.rateShow = p.fp_rate_show && p.fp_rate_show.replace('%', '<span class="small">%</span>');
            ...
            order.rateShow = p.fp_rate_show && p.fp_rate_show.replace('%', '');
            ...
            order.priceMax = parseInt(Math.min(+p.balance, +p.fp_price_max));
            order.price = +p.fp_price_min; //起投金额
            vm.order = order;
