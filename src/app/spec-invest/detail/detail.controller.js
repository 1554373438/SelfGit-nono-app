(function() {
  ...
  /** @ngInject */
  function SpecInvestDetailController(BasicApi, $scope, $stateParams, $state, toastr, localStorageService, BridgeService, BILog, OrderService, user, AppService, $log) {
    var vm = this,
    ...
    function getDetailInfo() {
      BasicApi.getSpecInvestDetailInfo({ 'productName': vm.productName }).success(function(res) {
        if (res.flag === 1) {
          vm.list = res.data;
    ...
    function buyProduct() {
      ...
      function callback() {
        if (terminal == 4 || terminal == 5) {
          ...
          BasicApi.getFinancePlan({ id: vm.product.fp_id }).success(function(res) {
            if (res.flag == 1) {
